...
# llm = Ollama(base_url = "http://10.1.0.133:11434", model="llama3", temperature=0.1)
...
def get_title_page_info(text):
    ...
    # instruction = "You are an assistant that specializes in answers questions only with the answer. Do not include anything that is not the answer in your response."
    # prompt = f""" 
    # Answer this question with only the answer: What would the labor category be for this government job? {title_info['job_title']}
    # """
    # response = client.chat.completions.create(
    #     model="gpt-4o-mini",
    #     messages=[
    ...
    #     ]
    # )
    # title_info['labor_category'] = response.choices[0].message.content.strip(".")
    return title_info
...
def generate_interview_questions(job_description, resume):
    ...
    """
    prompt = f""" 
    Generate 5 interivew questions tailored for this resume based on the job description in bullet point format: 
    {resume}
    """
    response = client.chat.completions.create(
        model="gpt-4o-mini",
...
def generate_proposal(resumes, concise_job_description, job_title):
    ...
    for resume in resumes:
        ...
        while len(resume_list) != 5 and retry_count < 5:
            formatted_resume = format_resume(resume)
            resume_list = formatted_resume.split("\n\n")
            retry_count += 1
        if resume_count == 0:
            data = {
                ...
                'resume_work_1' : resume_list[4].lstrip().rstrip()
            }
            ...
            data.update({
                ...
                'resume_work_2' : resume_list[4].lstrip().rstrip()
            })
            ...
            data.update({
                ...
                'resume_work_3' : resume_list[4].lstrip().rstrip()
            })
...
def main():
    ...
    if st.session_state["authentication_status"]:
        ...
        solicitation_file = st.file_uploader("Choose a solicitation PDF or docx file", type=[".pdf", ".docx"])
        if solicitation_file:
            button_col1, button_col2, button_col3 = st.columns([20,20,20])
            with button_col1:
                job_desc_button = st.button("Generate Job Description", key='enable_buttons')
            with button_col2:
                interview_question_button = st.button("Generate Interview Questions", disabled=st.session_state.disabled)
            with button_col3:
                resume_upload_btn = st.button("Generate Proposal using Template", disabled=st.session_state.disabled)
            ...
            if interview_question_button or st.session_state.interview_state:
            ...
            if resume_upload_btn or st.session_state.uploadbtn_state:
                ...
                if resume_files and template_file:
                    ...
                    if len(resume_files) > 3:
                        st.error("Please only select up to 3 resumes")
                        resume_over = True
                    gen_prop_button = st.button("Generate Proposal", disabled=resume_over)
                    if gen_prop_button:
